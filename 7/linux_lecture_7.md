...
### loop-пристрої : `/dev/loop*`

**Loop-пристрій** дозволяє Linux трактувати файл як пристрій блочного типу (аналогічно жорсткому диску). Це зручно для тестування файлових систем, розділів або образів дисків.

#### Перевірка наявних loop-пристроїв

Щоб побачити всі доступні loop-пристрої у системі:
...
#### Створення loop-пристрою

Припустимо, у вас є файл `disk_image.img`. Для асоціації файлу з loop-пристроєм використовуйте `losetup`:

```bash
sudo losetup --show -f disk_image.img
```

- Опція `--show` виводить назву прив’язаного loop-пристрою (наприклад, `/dev/loop0`).
- Опція `-f` знаходить перший доступний loop-пристрій.

#### Видалення прив’язки файлу до loop-пристрою

Щоб від’єднати файл від loop-пристрою:
