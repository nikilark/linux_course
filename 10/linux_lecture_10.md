...
Мережева підтримка в Linux — це набір програмних компонентів та інструментів, вбудованих в операційну систему Linux, які забезпечують функціональність мережевих з'єднань, конфігурацію мережевого інтерфейсу, маршрутизацію, мережеву безпеку та інші мережеві операції.
...
і дає змогу виконувати широкий спектр операцій, включно з керуванням станціями, налаштуванням параметрів, скануванням мереж, аналізом і моніторингом бездротової активності.

**`wpa_supplicant`** — управління Wi-Fi-з'єднаннями з підтримкою WPA/WPA2-шифрування

Позволяет сканировать доступные беспроводных сети и получать информацию о их параметрах и сигнале. Поддерживает настройку и управление различнми методами аутентификации, включая WPA-PSK, WPA-EAP и другие.
...
# Гілка параметрів і статистики IPv4
ll /proc/sys/net/ipv4/

# Гілка параметрів і статистики IPv6
...
# Отключаем в текущем состоянии системі
...
ss -t -o state '( ( state syn-sent or state fin-wait-1 ) and ( dport = :80 or dport = :443 ) )'
```
...
Для спрощення використання Інтернет, замість запам'ятовування числових IP-адрес користувачам пропонується більш зручна для взаємодії, система доменних імен. У системах Linux функції DNS (Domain Name System), виконує `systemd-resolved`, що є компонентом "systemd". Він замінює старіші інструменти та компоненти, такі як `resolv.conf`, `nsswitch.conf`, `gethostbyname` та інші.

##### **`resolvectl`** — перегляд і управління функціями системи DNS

Является частью пакета `systemd-resolved`.
...
```bash
# Резолв адреса IPv4 и IPv6 для одного или нескольких доменов
resolvectl query {{domain1 domain2 ...}}
resolvectl query i.ua google.com

# Получение домена указанного IP
resolvectl query {{ip_address}}
resolvectl query 8.8.8.8

# Получение записи MX домена
resolvectl --legend={{no}} --type={{MX}} query {{domain}}
resolvectl --legend=no --type=MX query outlook.com

# Резолв SRV-запись, например _xmpp-server._tcp gmail.com
resolvectl service _{{service}}._{{protocol}} {{name}}

# Получение открытого ключа от адреса электронной почты из DNS-записи OPENPGPKEY
...
Оскільки аапаратні годинники не підтримують концепцію часових поясів, то вона реалізується програмними засобами операційних систем.
...
```bash
# Определяем используемый в системе бекэнд менеджер
...
Для забезпечення безпеки міжмережевої взаємодії в ядро Linux вбудовано підсистему контролю мережевого тафіка та опрацювання правил фільтрації пакетів.
...
*Усе, що ви бачили досі, відбувається на транспортному рівні або нижче.*
...
В протоколе SSH ассиметричное шифрование используется в процессе обмена сеансовыми ключами, а также может участвует в процедуре аутентификации.

##### Функції хешування

Хеш-функції приймають вхідні дані довільної довжини і видають хеш фіксованої довжини. Це застосовується для

У протоколі SSH застосовується для перевірки автентичності "відбитка пальця" (fingerprint) віддаленого сервера, це дає змогу захиститися від атак класу "Людина по середині" (*Man-in-the-Middle*).
...
3. **Додайте правила файрволу (за необхідності)**: Якщо на вашому сервері встановлено файрвол, вам може знадобитися додати правила для дозволу вхідних підключень на порт, на якому працює SSH сервер (зазвичай порт 22). Наприклад, для утиліти `iptables` команда може виглядати так: `sudo iptables -A INPUT -p tcp --dport 22 -j ACCEPT`.
