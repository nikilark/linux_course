...
Головна функція комп'ютерних систем — виконання завдань інформаційного опрацювання даних. Це виконується за певним алгоритмом, який представлений у вигляді машинно-виконуваного бінарного коду (програм), або інтерпретованих сценаріїв (скриптів).
...
Поняття процесу повністю корелює з концепцією потоків даних, заведено говорити, що процес приймає потік даних на вхідний канал, обробляє їх, та повертає дані результату на канал вихідного потоку. Також процес наділяється якостями живої сутності — у нього є батьки та предки, він народжується та має життєвий цикл, у нього можна контролювати здоров'я, його можна вбити та навіть він може стати зомбі.
...
Завдяки контрольованому поділу ресурсів комп'ютера (процесорний час, пам'ять і введення-виведення), операційна система Linux здатна підтримувати кілька процесів одночасно. Це реалізується шляхом послідовного перемикання процесора між контекстами обробки різних процесів, фактично це псевдоодночасність.
...
4. **Multilevel Feedback Queue**: Процеси розподіляються по кількох чергах і виконуються залежно від їхнього пріоритету та інших факторів. Процеси можуть переміщуватися між різними рівнями черги залежно від їхньої поведінки.
...
### Фонові, інтерактивні та системні процеси
...
Команди, запущені користувачем через термінал, зазвичай виконуються на передньому плані та можуть блокувати термінал до їхнього завершення або призупинення.
...
- не прив'язані до термінала і не взаємодіють з користувачем безпосередньо
...
- ними можна керувати (зупинити, переконфігурувати та запустити) через системний процес init / systemd або через скрипти ініціалізації
...
- **Creation** — Створення нового процесу:
...
- **Running** — Виконання або обробка:
  ...
  - Процес активно виконується на процесорі та виконує свої завдання.
- **Waiting** — Очікування:
...
- **Terminated** — Завершення:
  ...
  - При цьому стані процес завершує свою роботу, звільняє займані ресурси та виставляє статус про своє завершення;
- **Zombie** — Зомбі
  ...
  - У цьому стані процес не виконує жодних операцій і вже звільнив усі займані ресурси;
...
Процес зупинено, але в цей момент до нього під'єднано дебагер.
...
- **Політикою** — У Linux планувальник процесів використовує алгоритми планування CFS (Completely Fair Scheduler) або інші, які можуть визначати різні політики та способи управління пріоритетами.
...
- `PRI` (Priority) — Абсолютний пріоритет процесу, фактично так його бачить ядро, впливати на нього немає можливості.
- `NI` (Nice) — це концепція простору користувача, наскільки процес "гарний", ним можна керувати через програми `nice` й `renice`.
...
### Керівні сигнали (Control Signals)

Для взаємодії між процесами в операційній системі є особливий механізм, який передає керівні сигнали активним процесам, та дає змогу керувати життєвим циклом процесу, опрацюванням помилок, зміною поведінки процесу тощо.

- Керівні сигнали, це програмні переривання, які ідентифікують за унікальним ім'ям або номером;
- У будь-який час процес може бути перерваний сигналом з ініціативи іншого процесу або ядра для обробки будь-якої події;
...
- **`SIGSTOP (19)`** — (Stop) зупинити процес
- **`SIGCONT (18)`** — (Continue) продовжити роботу після зупинки.
...
2. Керівні файли — використовується для передачі інформації ядру або його модулям.
...
- `schedstat` — статистика планувальника, наприклад, кількість часу виконання і часу очікування в черзі
...
- **`-a`** — інформація про всі процеси, крім групи процесів та тих, що не мають керівного термінала (_tty_)
- **`-x`** — інформація про процеси, які не мають керівного термінала
...
- **`TTY`** (Terminal) — керівний термінал, можливо псевдотермінал (pts) або консоль (tty), дивись команди **`who`** або **`w`**
...
- **`D`** (Uninterruptible Sleep) — призупинений, очікує завершення операції введення-виведення що блокує, і його не можливо перервати
...
**`nohup`** — створення умов фонового виконання програм без прив'язки до термінала
...
**`kill`** — надсилання керівних сигналів процесам відповідно за PID
...
**`pkill`** — надсилання керівних сигналів процесам відповідно за шаблоном імені
...
- **`fuser`** — (File User) управління процесами та блокуваннями файлів
